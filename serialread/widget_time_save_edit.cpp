...
//    }
    if(rawwave.size()>=3000)
    {
        rawwave.remove(0,2000);
        a++;
    }
    ...
    if(rawwave.size()>=30000)
    {
        ...
        if(file.open(QIODevice::Append | QIODevice::Text) )
        {
            ...
            for (int i=0; i <= edge-1 ;i++)
                out<<QString::number(rawwave.at(i))<<"\n";
            file.close();
        }
        qDebug()<<"saved";
    ...
    QFileDialog dlg(this);
        ...
        if( file.open(QIODevice::WriteOnly | QIODevice::Text) )
        {
            ...
            for(int i=0;i<a;i++)
            {
                ...
                {
                    buffer[j] = in.readLine();
                }
                temp.close();
                for(int j=0;j<edge;j++)
                {
                    out<<QString::number(time.at(i*edge+j))<<": "<<buffer[j]<<"\n";
                }
            }
            for (int i=0; i < rawwave.size() ;i++)
                out<<QString::number(time.at(a*edge + i))<<": "<<QString::number(rawwave.at(i))<<"\n";
